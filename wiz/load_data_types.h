...
namespace wiz {
	namespace load_data {
		...
		public:
			...
			// find userType! not itemList!,// this has bug
			static std::pair<bool, std::vector< UserType*> > Find2(UserType* global, const std::vector<std::string>& _strVec, StringBuilder* builder) /// option, option_offset
			{
				std::vector<UserType*> temp;
				std::vector<std::string> strVec;
				std::list<std::pair< UserType*, int >> utDeck;
				...
				for (int i = 0; i < _strVec.size(); ++i) {
					std::string strTemp = _strVec[i];
					if (strTemp == "root" && i == 0) {
					}
					else {
						strVec.push_back(strTemp);
